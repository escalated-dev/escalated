...
function unpinNote(note) {
    ...
    router.post(
        route(`${props.routePrefix}.tickets.pin`, [props.ticketReference, note.id]),
        {},
        {
            preserveScroll: true,
            onFinish: () => {
...
<template>
    <div
        ...
        :class="[
            ...
            escDark ? 'border-amber-500/20 bg-amber-500/[0.06]' : 'border-amber-300 bg-amber-50',
        ]"
    >
        <!-- Header -->
        <div class="mb-3 flex items-center gap-2">
            <svg
                :class="['h-4 w-4', escDark ? 'text-amber-400' : 'text-amber-600']"
                fill="none"
                stroke="currentColor"
                stroke-width="1.5"
                viewBox="0 0 24 24"
        ...
        <div class="space-y-3">
            <div
                ...
                :class="[
                    ...
                    escDark ? 'border-amber-500/10 bg-amber-500/[0.04]' : 'border-amber-200 bg-white',
                ]"
            >
                <!-- Note body -->
                <div
                    :class="[
                        ...
                        escDark ? 'prose-invert text-[var(--esc-panel-text-secondary)]' : 'text-gray-800',
                    ]"
                    v-html="sanitizeHtml(note.body)"
                ></div>
