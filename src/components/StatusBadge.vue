...
const props = defineProps({
    ...
    customStatus: { type: Object, default: null },
});
...
const config = computed(() => {
    // When a customStatus object is provided, use its label and color directly
    if (props.customStatus) {
        const hex = props.customStatus.color || '#6b7280';
        return {
            label: props.customStatus.label || props.status,
            color: '',
            customColor: hex,
        };
    }
        ...
        :style="
            config.customColor
                ? {
                      ...
                      boxShadow: 'inset 0 0 0 1px ' + config.customColor + '33',
                  }
                : {}
        "
    >
