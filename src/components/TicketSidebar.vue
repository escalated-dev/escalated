    ...
    <aside class="space-y-4">
        <div :class="cardClass">
            ...
            <dl class="space-y-2 text-sm">
                <div class="flex justify-between">
                    <dt :class="escDark ? 'text-[var(--esc-panel-text-muted)]' : 'text-gray-500'">Status</dt>
                    <dd><StatusBadge :status="ticket.status" /></dd>
                </div>
                <div class="flex justify-between">
                    <dt :class="escDark ? 'text-[var(--esc-panel-text-muted)]' : 'text-gray-500'">Priority</dt>
                    <dd><PriorityBadge :priority="ticket.priority" /></dd>
                </div>
                <div class="flex justify-between">
                    <dt :class="escDark ? 'text-[var(--esc-panel-text-muted)]' : 'text-gray-500'">Reference</dt>
                    <dd :class="['font-mono text-xs', escDark ? 'text-[var(--esc-panel-text)]' : '']">
                        {{ ticket.reference }}
                    </dd>
                </div>
                ...
                <div class="flex justify-between">
                    <dt :class="escDark ? 'text-[var(--esc-panel-text-muted)]' : 'text-gray-500'">Created</dt>
                    <dd :class="escDark ? 'text-[var(--esc-panel-text-secondary)]' : ''">
        ...
        <div v-if="ticket.satisfaction_rating" :class="cardClass">
            ...
            <div class="flex items-center gap-1">
                <svg
                    v-for="(star, i) in renderStars(ticket.satisfaction_rating.rating)"
                    :key="i"
                    class="h-5 w-5"
                    :class="
                        star === 'filled'
            ...
            <p
                v-if="ticket.satisfaction_rating.comment"
                :class="['mt-2 text-xs italic', escDark ? 'text-[var(--esc-panel-text-muted)]' : 'text-gray-500']"
            >
        ...
        <div v-if="editable && tags.length" :class="cardClass">
            <TagSelect
                :tags="tags"
                :model-value="(ticket.tags || []).map((t) => t.id)"
