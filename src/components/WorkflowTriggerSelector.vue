...
import { DEFAULT_TRIGGER_EVENTS, normalizeOptions } from '../utils/workflowContract.js';

const props = defineProps({
    modelValue: { type: String, default: '' },
    // The events the backend fires, in any shape normalizeOptions accepts.
    // Absent means the contract's five canonical triggers.
...
const DEFAULT_ICON = 'M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z';

// Descriptions and icons for the events backends are known to fire. An event
// not listed here still renders, with its derived label and a generic icon.
const DETAILS = {
    'ticket.created': {
    ...
    'reply.created': { description: 'When a reply is posted on a ticket', icon: REPLY_ICON },
    'ticket.replied': { description: 'When a reply is posted on a ticket', icon: REPLY_ICON },
    'ticket.escalated': {
...
const triggers = computed(() =>
    normalizeOptions(props.options, DEFAULT_TRIGGER_EVENTS).map((option) => ({
        ...option,
        description: DETAILS[option.value]?.description ?? '',
        icon: DETAILS[option.value]?.icon ?? DEFAULT_ICON,
    })),
);
...
<template>
    <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        ...
        >
            ...
            <span v-if="trigger.description" class="mt-1 text-[10px] leading-tight text-[var(--esc-panel-text-muted)]">
                {{ trigger.description }}
            </span>
            <div
