...
<template>
    <EscalatedLayout title="Agent Dashboard">
        ...
        <h2 class="mb-4 text-lg font-semibold text-gray-200">Recent Tickets</h2>
            ...
            <table class="min-w-full divide-y divide-white/[0.06]">
                <thead>
                    <tr class="bg-white/[0.02]">
                        ...
                        <th class="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-500">Requester</th>
                        ...
                        <th class="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-500">Assignee</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-white/[0.04]">
                    <tr v-for="ticket in recentTickets" :key="ticket.id" class="transition-colors hover:bg-white/[0.03]">
                        <td class="px-4 py-3 text-sm">
                            <a :href="route('escalated.agent.tickets.show', ticket.reference)" class="font-medium text-cyan-400 hover:text-cyan-300">{{ ticket.reference }}</a>
                        </td>
                        <td class="px-4 py-3 text-sm text-gray-200">{{ ticket.subject }}</td>
                        <td class="px-4 py-3 text-sm text-gray-400">{{ ticket.requester?.name }}</td>
                        <td class="px-4 py-3 text-sm">
                            <span :class="[
                                'inline-flex rounded-full px-2 py-0.5 text-xs font-medium ring-1',
                                {
                                    'bg-cyan-500/10 text-cyan-400 ring-cyan-500/20': ticket.status === 'open',
                                    ...
                                    'bg-gray-500/10 text-gray-400 ring-gray-500/20': ticket.status === 'closed',
                                }
                            ]">{{ ticket.status }}</span>
                        </td>
                        <td class="px-4 py-3 text-sm capitalize text-gray-400">{{ ticket.priority }}</td>
                        <td class="px-4 py-3 text-sm text-gray-400">{{ ticket.assignee?.name || 'Unassigned' }}</td>
                    </tr>
                    <tr v-if="!recentTickets?.length">
                        <td colspan="6" class="px-4 py-8 text-center text-sm text-gray-500">No recent tickets</td>
                    </tr>
                </tbody>
            </table>
        </div>
