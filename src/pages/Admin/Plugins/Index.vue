...
const activePlugins = computed(() => props.plugins.filter((p) => p.is_active));
const inactivePlugins = computed(() => props.plugins.filter((p) => !p.is_active));

function activate(slug) {
    router.post(
        route('escalated.admin.plugins.activate', slug),
        {},
        {
            preserveScroll: true,
        },
...
function deactivate(slug) {
    router.post(
        route('escalated.admin.plugins.deactivate', slug),
        {},
        {
            preserveScroll: true,
        },
...
function confirmDelete(plugin) {
    confirmingDelete.value = plugin.slug;
}
...
function deletePlugin(plugin) {
    router.delete(route('escalated.admin.plugins.destroy', plugin.slug), {
        preserveScroll: true,
        onSuccess: () => {
            confirmingDelete.value = null;
        },
...
function handleUpload(event) {
    ...
    router.post(
        route('escalated.admin.plugins.upload'),
        { plugin: file },
        {
            forceFormData: true,
            preserveScroll: true,
...
function statusColor(plugin) {
    if (plugin.is_active) return 'bg-emerald-500';
    return 'bg-neutral-600';
}

function statusLabel(plugin) {
    return plugin.is_active ? 'Active' : 'Inactive';
}
</script>
                    ...
                    <div class="flex shrink-0 items-center gap-2">
                        ...
                        <button
                            :class="[
                                ...
                                plugin.is_active ? 'bg-emerald-500' : 'bg-neutral-700',
                            ]"
                            :title="plugin.is_active ? 'Deactivate' : 'Activate'"
                            @click="plugin.is_active ? deactivate(plugin.slug) : activate(plugin.slug)"
                        >
                            <span
                                :class="[
                                    'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition-transform',
                                    plugin.is_active ? 'translate-x-5' : 'translate-x-0',
                                ]"
                            />
                        </button>

                        <!-- Delete (local plugins only) -->
                        <template v-if="plugin.source !== 'composer'">
                            <template v-if="confirmingDelete === plugin.slug">
                                <button
