...
const props = defineProps({
    ...
    median_frt: { type: Number, default: 0 },
    p90_frt: { type: Number, default: 0 },
    ...
    target_hours: { type: Number, default: 4 },
});
...
function changePeriod(days) {
    ...
    router.get(route('escalated.admin.reports.response-times'), { days }, { preserveState: true });
}

function frtColor(value) {
...
<template>
    <EscalatedLayout title="First Response Time Analysis">
        <div class="mb-6 flex items-center justify-between">
            <PeriodSelector :model-value="periodDays" @update:model-value="changePeriod" />
            <div class="flex items-center gap-3">
                <ReportExportButton report-name="response-times" :period-days="periodDays" />
                <Link
        ...
        <div class="mb-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
            <ReportChart
                ...
                :data="distribution"
                :colors="['#06b6d4']"
                x-label="Response Time Buckets"
            />
            <ReportChart type="line" title="Average FRT Trend" :data="trend" :colors="['#8b5cf6']" />
            <ReportChart type="bar" title="FRT by Department" :data="by_department" :colors="['#f59e0b']" />
            <ReportChart type="bar" title="FRT by Priority" :data="by_priority" :colors="['#10b981']" />
        </div>
        ...
        <div class="overflow-hidden rounded-xl border border-[var(--esc-panel-border)]">
            <div class="border-b border-[var(--esc-panel-border)] bg-[var(--esc-panel-hover)] px-4 py-3">
                <h3 class="text-sm font-semibold text-[var(--esc-panel-text-secondary)]">FRT by Agent</h3>
            </div>
            <table class="w-full">
                <thead>
                ...
                <tbody class="divide-y divide-[var(--esc-panel-border)]">
                    <tr v-for="agent in sortedAgents" :key="agent.agent_id" class="hover:bg-[var(--esc-panel-hover)]">
                        ...
                        <td class="px-4 py-3 text-sm font-medium" :class="frtColor(agent.avg)">
                            {{ agent.avg ?? '—' }}h
                        </td>
                        <td class="px-4 py-3 text-sm font-medium" :class="frtColor(agent.median)">
                            {{ agent.median ?? '—' }}h
                        </td>
                        <td class="px-4 py-3 text-sm font-medium" :class="frtColor(agent.p90)">
                            {{ agent.p90 ?? '—' }}h
                        </td>
                    </tr>
