    ...
    <EscalatedLayout title="Reports">
        ...
        >
            ...
            <Link
                ...
                class="rounded-md px-3 py-1.5 text-sm text-[var(--esc-panel-text-tertiary)] transition-colors hover:bg-[var(--esc-panel-hover)] hover:text-[var(--esc-panel-text-secondary)]"
            >
                Dashboard
            </Link>
            <Link
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            <div class="rounded-xl border border-[var(--esc-panel-border)] bg-[var(--esc-panel-surface)] p-5">
                <h3 class="mb-4 text-sm font-semibold text-[var(--esc-panel-text-secondary)]">By Status</h3>
                <template v-if="by_status && Object.keys(by_status).length">
                    ...
                    >
                        <span class="text-sm capitalize text-[var(--esc-panel-text-tertiary)]">{{
                            status.replace('_', ' ')
                        }}</span>
                        <span class="text-sm font-semibold text-[var(--esc-panel-text)]">{{ count }}</span>
                    </div>
                </template>
                <div v-else class="flex flex-col items-center py-6 text-center">
                    <svg
                        class="mb-2 h-8 w-8 text-[var(--esc-panel-text-muted)]"
                        fill="none"
                        stroke="currentColor"
                        stroke-width="1.5"
                        viewBox="0 0 24 24"
                    ...
                    <p class="text-sm text-[var(--esc-panel-text-muted)]">No ticket data for this period</p>
                </div>
            </div>
            <div class="rounded-xl border border-[var(--esc-panel-border)] bg-[var(--esc-panel-surface)] p-5">
                <h3 class="mb-4 text-sm font-semibold text-[var(--esc-panel-text-secondary)]">By Priority</h3>
                <template v-if="by_priority && Object.keys(by_priority).length">
                    <div
                        v-for="(count, priority) in by_priority"
                        :key="priority"
                        class="mb-2.5 flex items-center justify-between"
                    >
                        <span class="text-sm capitalize text-[var(--esc-panel-text-tertiary)]">{{ priority }}</span>
                        <span class="text-sm font-semibold text-[var(--esc-panel-text)]">{{ count }}</span>
                    </div>
                </template>
                <div v-else class="flex flex-col items-center py-6 text-center">
            ...
            <!-- CSAT -->
            <div class="rounded-xl border border-[var(--esc-panel-border)] bg-[var(--esc-panel-surface)] p-5">
                <h3 class="mb-4 text-sm font-semibold text-[var(--esc-panel-text-secondary)]">Customer Satisfaction</h3>
                <template v-if="csat?.total > 0">
                    <div class="mb-3 flex items-center gap-2">
                        <div class="flex items-center gap-0.5">
                            <svg
                                v-for="(star, i) in renderStars(csat.average)"
                                :key="i"
                                class="h-5 w-5"
                                :class="star === 'filled' ? 'text-amber-400' : 'text-[var(--esc-panel-text-muted)]'"
                                fill="currentColor"
                                viewBox="0 0 20 20"
                            >
                                <path
                        ...
                        <span class="text-lg font-bold text-[var(--esc-panel-text)]">{{ csat.average }}</span>
                        <span class="text-sm text-[var(--esc-panel-text-muted)]">({{ csat.total }} ratings)</span>
                    </div>
                    <div v-if="csat.breakdown && Object.keys(csat.breakdown).length" class="space-y-1.5">
                        <div v-for="rating in [5, 4, 3, 2, 1]" :key="rating" class="flex items-center gap-2">
                            <span class="w-3 text-xs text-[var(--esc-panel-text-muted)]">{{ rating }}</span>
                            <div class="h-2 flex-1 overflow-hidden rounded-full bg-[var(--esc-panel-hover)]">
                                <div
                                    class="h-full rounded-full bg-amber-400"
                                    :style="{
                                        width: `${csat.total ? ((csat.breakdown[rating] || 0) / csat.total) * 100 : 0}%`,
                                    }"
                                ></div>
                            </div>
                            <span class="w-6 text-right text-xs text-[var(--esc-panel-text-muted)]">{{
                                csat.breakdown[rating] || 0
                            }}</span>
                        </div>
                    </div>
                </template>
                <div v-else class="flex flex-col items-center py-6 text-center">
                    ...
                    <p class="text-sm text-[var(--esc-panel-text-muted)]">No ratings for this period</p>
                </div>
            </div>
        </div>
        <PluginSlot slot="admin.reports.footer" :components="getPageComponents('admin.reports', 'footer')" />
    </EscalatedLayout>
</template>
