...
test('README hero: Admin Dashboard Light (escalated_admin_3)', async ({ page }) => {
    await openStory(page, 'pages-admindashboard--full-dashboard-light');
    await page.evaluate(() => {
        document.body.style.background = 'transparent';
        document.documentElement.style.background = 'transparent';
        const root = document.querySelector('#storybook-root');
    ...
    await page.screenshot({
        path: 'docs/assets/escalated_admin_3.png',
        animations: 'disabled',
        omitBackground: true,
    });
});
...
test('README hero: Ticket Detail View (escalated_ticket_view)', async ({ page }) => {
    await openStory(page, 'pages-admindashboard--ticket-detail-view');
    await page.evaluate(() => {
        document.body.style.background = 'transparent';
        document.documentElement.style.background = 'transparent';
        const root = document.querySelector('#storybook-root');
    ...
    await page.screenshot({
        path: 'docs/assets/escalated_ticket_view.png',
        animations: 'disabled',
        omitBackground: true,
    });
});
